...
public class SetupCommands {
	...
	private CloudAccount getCloudAccount(String in, PipeOut out, List<CloudAccount> cloudList){
		if(cloudList.size() == 0){
    	...
    	int choiceIdx = prompt.promptChoice("Choose a cloud account or N for a new cloud", choices);
	...
	private Environment createEnvironment(String in, PipeOut out, List<CloudAccount> cloudList, CloudAccount lastCloudCreated){
		out.println("You do not have any environments, creating a new one...");
        out.println();
        if(lastCloudCreated != null){
            shell.execute("rhc create-environment --cloudId " + lastCloudCreated.getId());
	...
	private Environment getEnvironment(String in, PipeOut out, List<CloudAccount> cloudList, CloudAccount cloudAccount)
		...
		List<Environment> environments = environmentDao.listEnvironments(ssoCookie, flexHost, flexContext);
	...
	private Application getApplication(String in, PipeOut out, Environment environment) 
        ...
        if(applications.size()==0){
        	...
        	choices.add("N");
        	int choiceIdx = prompt.promptChoice("Choose an application GUID or N for a new application", choices);
        	if(choices.get(choiceIdx).equalsIgnoreCase("N")){
        		return createApplication(in,out,environment);
        	}else{
	...
	private Application createApplication(String in, PipeOut out, Environment environment) {
		out.println("You do not have any environments, creating a new one...");
        out.println();
        shell.execute("rhc create-application --environmentId " + environment.getId());
	...
	public void setup(String in, PipeOut out, Properties rhcProperties){
		...
		if(!rhcProperties.containsKey("rhlogin")){
			if(prompt.promptBoolean("Remember login name?")){
				base.get().saveRhcProperties("rhlogin", "\"" + base.get().getUsername() + "\"");
			}
		}

        out.println("This wizard will help you set up a new cloud account, environment and application template for you JEE application");
        out.println();
		try {
            CloudAccount cloudAccount = null;
			List<CloudAccount> cloudList = cloudAccountDao.listClouds(ssoCookie, flexHost, flexContext);
			...
			Environment environment = getEnvironment(in,out,cloudList,cloudAccount);
			Application application = getApplication(in,out,environment);
			
			MavenCoreFacet maven = project.getFacet(MavenCoreFacet.class);
			Model pom = maven.getPOM();
			Properties props = pom.getProperties();
			props.setProperty("com.openshift.application", application.getGuid());
